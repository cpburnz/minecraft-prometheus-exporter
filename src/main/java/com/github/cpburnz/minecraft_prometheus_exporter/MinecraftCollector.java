...
public class MinecraftCollector extends Collector implements Collector.Describable {
    ...
    private GaugeMetricFamily collectPlayerList() {
        ...
        for (String player : this.mc_server.getConfigurationManager()
            .getAllUsernames()) {
            // Removed UUID since that caused BIG problems if player was not in default dim
            String id_str = "";
            metric.addMetric(Arrays.asList(id_str, player), 1);
        }
        return metric;
    }
